...
#	python "$DIR"/../../pico_galaxy/tools/get_orfs_or_cdss/get_orfs_or_cdss.py -i <(tr -d '\000' < $CURDIR/$file) -e open -m all --min_len $aalength --op ORF_prot.fasta
	...
	# ORFfinder does not like to have '|' in the sequence ID,
	# so replace them with something we won't hopefully encounter
	"$DIR"/../../Tools/ORFfinder \
		-in <((tr -d '\000' < $CURDIR/$file) | tr '|' '@') \
		-s 2 \
		-ml $ntlength \
		-n true \
		-out ORF_prot1.fasta \
	...
	sed -i 's/lcl\|//g' ORF_prot1.fasta
	sed -i 's/\@/\|/g' ORF_prot1.fasta
